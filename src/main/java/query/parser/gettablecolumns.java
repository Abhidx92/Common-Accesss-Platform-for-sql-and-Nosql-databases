...
class metaDB implements IMetaDatabase {
	...
	String columns[][] = { { "dbo", "subselect3table1", "f1" }, { "dbo", "subselect3table2", "s3t1a1" } };
	...
	@Override
	public boolean checkColumn(String arg0, String arg1, String arg2, String arg3, String arg4) {
	...
	String run(String query) {
		...
		if (ret == 0) {
			...
			/*for (int k1 = 0; k1 < foundColumnsCount; k1++) {
				foundColumns[k1] = foundColumns[k1];
			}*/
			...
			for (int j = 0; j < foundColumns2.length; j++) {
				// System.out.println(foundColumns3[j]);
				stringBuffer.append(foundColumns2[j] + "\n");
			}
		...
		for (int i = 0; i < stmt.tables.size(); i++) {
			if (stmt.tables.getTable(i).isBaseTable()) {
				...
				for (int j = 0; j < stmt.tables.getTable(i).getObjectNameReferences().size(); j++) {
					...
					foundColumns[foundColumnsCount] += "(table determined:"
							+ stmt.tables.getTable(i).getObjectNameReferences().getObjectName(j).isTableDetermined()
							+ ")";
					foundColumnsCount++;
				}
			}
...
public class gettablecolumns {
	 ...
	 */
	 public String parse(String query) {
		...
		//String query = "INSERT INTO Customers (CustomerName, ContactName, Address, City, PostalCode, Country) VALUES ('Cardinal', 'Tom B. Erichsen', 'Skagen 21', 'Stavanger', '4006', 'Norway');";
...
/*
		...
		System.out.println("Select" + "ed SQL dialect: " + dbVendor.toString());

		getObject g = new getObject(dbVendor);
		//System.out.println(g.run(query));
		 return g.run(query);
